...
FAISS_PATH = "faiss_index"  # Repo-relative for Streamlit Cloud
...
def load_resume_model():
    try:
        ...
        st.error(f"❌ Resume model failed: {e}")
        return None
...
def load_embeddings_and_faiss():
    """Load FAISS if exists, else build from dataset automatically."""
    ...
    # Else build FAISS from dataset
    df = load_data()
    if df.empty:
        return emb_model, None

    vect = FAISS.from_texts(df['combined'].tolist(), embeddings=emb_model)
...
def build_rag_chain():
    ...
    if vectorstore is None:
        st.error("❌ FAISS vector store not found or empty. Please check your dataset.")
        return None
...
# ---------- Resume Matcher ----------
    ...
    if uploaded_file:
        ...
        if resume_model and df is not None and resume_text:
